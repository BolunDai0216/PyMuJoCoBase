...
class ManipulatorIK(MuJoCoBase):
    ...
    def forward_kinematics(self, q):
        ...
        end_eff_pos = np.array(
            [self.data_sim.sensordata[0], self.data_sim.sensordata[2]]
        )
    ...
    def simulate(self):
        while not glfw.window_should_close(self.window):
            ...
            end_eff_pos = np.array([self.data.sensordata[0], self.data.sensordata[2]])
    ...
    def visualize(self):
        ...
        leminiscate_z = self.center_z + (self.a * np.sin(wt) * np.cos(wt)) / denominator
        ...
        ax.grid()
        ax.set_aspect("equal")
        ax.legend(frameon=False, ncol=2, loc="lower center", bbox_to_anchor=(0.5, -0.4))

        plt.savefig(
            "imgs/manipulator_ik.png", dpi=200, transparent=False, bbox_inches="tight"
        )


def main():
    data_path = getDataPath()
    xml_path = data_path + "/xml/manipulator.xml"
